...
- **Bandit camp** — built at your direction (two tiles north + one west of the borderlands ambush, at surface coords (8,8), with two new transitional desert tiles in between). Layout: campfire at the center, a new `BanditLeader` (level 4, placeholder stats — high dex/agility, pending your review) plus 2 `Bandit` and 3 `BanditArcher` sitting close around it, 4 simple `Tent` props ringed further out, and rock/ruin-block cover scattered for stealth approach lanes and arrow cover. **Stats not finalized** — you asked to review all the bandits' stats together once the layout was done; that review is still pending.
