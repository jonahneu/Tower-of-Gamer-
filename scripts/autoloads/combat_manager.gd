...
func _entity_can_see_player(entity: Node, player: Node) -> bool:
	...
	# In full or dim light, an already-engaged entity tracks the player by
	# line of sight alone — outrunning its aggro range shouldn't break
	# tracking; only an actual LOS block (wall, corner, closed door) counts
	# as the player losing them. Genuine darkness on either side is the
	# exception: it's dim enough that simply getting far away — no LOS break
	# needed — is enough to vanish, same as the light-based cap that gates
	# initial detection (see LightLevels.vision_cap).
	if observer_level != LightLevels.DARK and target_level != LightLevels.DARK:
